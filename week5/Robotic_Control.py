...
# ===============================================================
print("\n[任务1] 笛卡尔空间 —— 画圆")
print("  圆心: (0.9, 0.0, 1)  半径: 0.2m")
p.removeUserDebugItem(label_id)
label_id = add_label("Task 1: Cartesian Circle (IK)", [0.0, 0.4, 0.9])

# 圆的参数
cx, cy, cz = 0.9, 0.0, 1   # 圆心（笛卡尔坐标）
radius = 0.2             # 半径（米）
total_angle = 4 * math.pi       # 整圆
circle_steps = 800              # 步数越多越平滑
...
start_pos = p.getLinkState(pandaId, 11)[0]

end_pos = [start_pos[0] + 0.4, start_pos[1], start_pos[2]]
...
for step in range(steps + 1):
    ...
    trail_line.append(p.getLinkState(pandaId, 11)[0])
