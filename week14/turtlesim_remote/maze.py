...
“编织”（braid）随机打通一部分墙，制造**多条通路与环路**——这样最短路径
不再唯一，必须用 BFS / Dijkstra / A* 这类**路径搜索算法**才能找到最优解，
而不是靠“沿墙走”蒙过去。

build_maze() 返回一个 dict：
    obstacles : [{x,y,w,h}]    墙体矩形（左下角 + 宽高），喂给桥接程序碰撞
    bounds    : {min_x,...}     画布边界
    start/goal: {x,y,...}       起点/终点世界坐标
    grid      : {cols,rows,origin,cell}
    start_cell/goal_cell : (i,j)
    neighbors(cell) -> [cell]   返回与某格连通的相邻格（给搜索算法用）
"""
import random
...
TURTLE_RADIUS = 0.45
SEED = 23
BRAID = 0.18          # Extra passages ratio: 0=perfect maze, higher values add loops.
